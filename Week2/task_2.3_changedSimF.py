...
def main(gt_path, det_dir, output_file):
    """Main function to compute HOTA for multiple detection files based on one ground truth file."""
    
    # Open the output file in write mode (it will overwrite the file or create a new one)
    with open(output_file, "w") as f:
        # Write a header to the output file
        f.write("Results for all detection files:\n\n")

        # Loop through each detection file in the directory
        for det_filename in os.listdir(det_dir):
            if not det_filename.endswith(".txt"):
                continue  # Skip non-TXT files
            
            # Construct the full path for the detection file
            ...
            identity_results = identity_metric.eval_sequence(data)

            # Write the results to the single output file
            f.write(f"Results for {det_filename}:\n")
            f.write(f"HOTA Score: {hota_results['HOTA']}\n")
            f.write(f"Identity Score (IDF1): {identity_results['IDF1']}\n")
            f.write("\n" + "="*50 + "\n\n")
