...
def save_tracks_iou(tracks, output_file, frame_id):
...
def save_tracks_kalman(tracks, output_file, frame_id):
...
def track_objects_with_iou_pyflow(detection_file, video_path, output_file_iou_pyflow, output_file_kalman_pyflow,
                                  iou_threshold=0.1, confidence_threshold=0.6, grace_period=1):
    ...
    next_track_id = 242
    missed_frame_count = {}
    open(output_file_iou_pyflow, "w").close()
    ...
    tracker = Sort()
    open(output_file_kalman_pyflow, "w").close()
    ...
    for frame_id in sorted(detections.keys()):
        ...
        save_tracks_kalman(tracked_objects, output_file_kalman_pyflow, frame_id)
        ...
        save_tracks_iou(tracks, output_file_iou_pyflow, frame_id)
...
output_file_iou_pyflow = "task_1_2_tracking_results/track_iou_pyflow.txt"
output_file_kalman_pyflow = "task_1_2_tracking_results/track_kalman_pyflow.txt"
...
# Run tracking
track_objects_with_iou_pyflow(detection_file, video_path, output_file_iou_pyflow, output_file_kalman_pyflow)
