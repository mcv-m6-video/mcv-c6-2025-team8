...
from model.model_bi_lstm_tdeed import Model
# Import wandb
import wandb
...
def update_args(args, config):
    ...
    args.num_workers = config['num_workers']
     ...
     # >>> Added for T-DEED inspired features
    args.use_displacement = config.get('use_displacement', False)
    args.mixup_alpha = config.get('mixup_alpha', 0.0)
    args.use_amp = config.get('use_amp', False)
...
def main(args):
    ...
    wandb.init(project="C6_W6", entity="C3_MCV_LGVP", name="bi_lstm+tdeed, aplha=0.2, w. displacement")
    ...
    # Log mAP and per-class AP to W&B
    wandb.log({"test_mAP": map_score})

    for i, class_name in enumerate(classes.keys()):
        wandb.log({f"AP/{class_name}": ap_score[i]})
